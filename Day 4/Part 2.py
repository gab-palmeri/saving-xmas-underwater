...
def check_if_winning(has_won, card_index, single_card, matrix_mapped, last_number):

    #Iterate through all the rows of the mapped matrix
    for row in matrix_mapped:
        #Check if the card won: if so, mark it
        if(all(el == True for el in row)):
            has_won[card_index] = True

            #Check if it was the last one to win: if so, calculate the score
            if(all(el == True for el in has_won)):
                winning = 0
...
with open("input_day4.txt") as input_file:
    ...
    #Array to monitor the winning cards
    has_won = []
            ...
            for row_index, row in enumerate(single_card):
                for el_index, element in enumerate(row):
                    ...
                    if(element == single_number):
                        ...
                        check_if_winning(has_won, card_index, single_card, matrix_mapped[card_index], single_number) == True
